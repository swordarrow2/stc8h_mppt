...
void PWMA_Init(void) {
    ...
    PWM_UpdateDuty(PWMA_CH3, 0.0f, PWM_MODE1); //设置通道的PWM占空比
...
void PWM_UpdateDuty(uint8_t PWMx, float fDuty, uint8_t iMode) {
    if (fDuty <= (0.0f + FLT_EPSILON)) {
        ...
        switch (PWMx) {
            case PWMA_CH1:
                PWMA_OC1REFForceHigh();
                break;
            case PWMA_CH2:
                PWMA_OC2REFForceHigh();
                break;
            case PWMA_CH3:
                PWMA_OC3REFForceHigh();
                break;
            case PWMA_CH4:
                PWMA_OC4REFForceHigh();
                break;
            case PWMB_CH5:
                PWMB_OC5REFForceHigh();
                break;
            case PWMB_CH6:
                PWMB_OC6REFForceHigh();
                break;
            case PWMB_CH7:
                PWMB_OC7REFForceHigh();
                break;
            case PWMB_CH8:
                PWMB_OC8REFForceHigh();
                break;
        }
    } else {
        uint16_t arr;
...
uint16_t PWM_ReadCapture(uint8_t PWMx) {
    ...
    switch (PWMx) {
        case PWMA_CH1:
            cap = PWMA_ReadCCR1Value();
            break;
        case PWMA_CH2:
            cap = PWMA_ReadCCR2Value();
            break;
        case PWMA_CH3:
            cap = PWMA_ReadCCR3Value();
            break;
        case PWMA_CH4:
            cap = PWMA_ReadCCR4Value();
            break;
        case PWMB_CH5:
            cap = PWMB_ReadCCR5Value();
            break;
        case PWMB_CH6:
            cap = PWMB_ReadCCR6Value();
            break;
        case PWMB_CH7:
            cap = PWMB_ReadCCR7Value();
            break;
        case PWMB_CH8:
            cap = PWMB_ReadCCR8Value();
            break;
        default:
            cap = 0;
    }
